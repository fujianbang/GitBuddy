...
pub fn openai_request(diff_content: &str) -> Result<String> {
    ...
    return if response.status().is_success() {
        ...
        println!("Usage: {}(completion: {}, prompt: {})",
        ...
        if response_json.choices.get(0).unwrap().message.content != "".to_string() {
            Ok(response_json.choices.get(0).unwrap().message.content.clone().trim().to_string())
        } else {
